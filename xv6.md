...
* 突然想明白了，fork完之后，父子进程都是相同的pagetable以及内存，执行exec需要把子进程的所有都释放掉(也不是释放，清空？)，然后赋值上要执行的程序各种东西。

***

## Lecture 9

之前提到在内核地址空间中，低于0x8000的都是硬件设备，如果访问的va是这些位置，那么就会访问到不同的硬件设备(主板的设计人员决定)。其中：
